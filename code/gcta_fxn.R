Rgcta <- function( A, y, X, verbose=TRUE, trunc=T, M.all,
	gcta_loc='~/mfmr/gcta_code/gcta64 ',
	#IDs=1:length(y),
	...
	write_gcta_pheno( y, file=paste0( tmpdir, '/y.phen')	, IDs=1:length(y) )
	write_gcta_pheno( X, file=paste0( tmpdir, '/X.qcovar'), IDs=1:length(y) )

	### write each of the A to a kinship file
	for( i in 1:r )
		write_gcta_kinship( A[[i]], prefix=paste0( tmpdir, '/K.', i ), IDs=1:length(y), M.all=M.all )
