rm( list=ls() )
library(GxEMM)
load( '../sims/Rdata/setup.Rdata' )
source( '../code/plot_fxns.R' )
maxit	<- 500


testmodes	<- c( 'wald', 'lrt', 'HE' )
pdfs			<- c( '~/figs/gxemm/Fig2_small_N.pdf', '~/figs/gxemm/Fig2_small_N_LRT.pdf', '~/figs/gxemm/Fig2_small_N_HE.pdf' )
legsubs		<- list(
	c(1,2,3,4,5,6,7,8),
	c(3,4,5,8),
	c(1,2,3,4,5,6,7,8)
...
for( sigtype in c(1:3,5,4) ){
	...
	for( ii in c( 'h2eq', 'iid', 'diag1', 'diag4', 'hom', 'hom1', 'diag3' ) )
