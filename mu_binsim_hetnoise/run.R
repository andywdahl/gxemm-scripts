...
try({
	...
	# generate pheno
	y		<- simfxn( it, X=cbind(1,X), Z, G, sig2hom=.35, sig2het=rep(0,K0), tauhet=c( rev(xs)[xval],  xs[xval] ), bin=TRUE, alpha1=.4, fixmean=TRUE )
	rm( G )
	...
	tmpdir  <- paste0( '/wynton/scratch/gxemm/tmpdir_', mode, '_', it, '_', xval, '_hetnoise_bin' )

	if( ! file.exists( savefile1 ) ){
		out_hom		<- GxEMM( y, X, K, Z, binary=( mode == 'pcgc' ), gtype='hom', tmpdir=tmpdir )
		save( out_hom, file=savefile1 )
	}
	if( ! file.exists( savefile2 ) ){
		out_het		<- GxEMM( y, X, K, Z, binary=( mode == 'pcgc' ), gtype='iid', tmpdir=tmpdir )
		save( out_het, file=savefile2 )
	}
	if( ! file.exists( savefile3 ) ){
		out_diag	<- GxEMM( y, X, K, Z, binary=( mode == 'pcgc' ), gtype='free', tmpdir=tmpdir )
		save( out_diag, file=savefile3 )
	}
	if( ! file.exists( savefile4 ) ){
		out_diag1	<- GxEMM( y, X, K, Z, binary=( mode == 'pcgc' ), gtype='free', etype='free', tmpdir=tmpdir )
