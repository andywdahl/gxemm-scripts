...
pdf( '~/figs/gxemm/Fig10.pdf', width=15.5, height=4.2 )
...
for( sigtype in c(1:3,5,4) ){
	...
	if( sigtype == 4 ){
		plot( c(.08,.62), c(0,.14), type='n', axes=F, xlab='', main='', ylab='', cex.lab=2 )
	} else {
		plot( xlim			, c(0,.14), type='n', axes=F, xlab='', main='', ylab='', cex.lab=2 )
	}
	box()
