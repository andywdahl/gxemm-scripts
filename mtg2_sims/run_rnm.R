...
for( xval in sample(nx) ){
	...
	savefile	<- paste0( 'Rdata/rnm/'	, xs[xval], '_', it, '.Rdata' )
	sinkfile	<- paste0( 'Rout/rnm_'	, xs[xval], '_', it, '.Rout' )
	...
	if( ! file.exists( savefile ) ){
		mrnmtime_rnm	<- system.time({
			out_rnm		<- Rmtg2( y=y, X=X, Z=Z, suffix=paste0( '.', it, '_', xval ), mc.cores=1, type='rnm', IDs=IDs )
		})[3]
		print( out_rnm )
		save( out_rnm, mrnmtime_rnm, file=savefile )
	}

	rm( y, out_rnm, mrnmtime_rnm, Z, X )
	sink()
	print(warnings())
	print('Done')
}
