...
pdf( paste0( '~/figs/gxemm/Fig3_alt.pdf' ), width=12.5, height=5.6 )
...
for( mode in modes ){
	...
	lines( xs, xs*0   +.15, col='grey' , lwd=3, lty=3 )
	lines( xs,     xs +.15, col=cols[4], lwd=3, lty=3 )
	lines( xs, rev(xs)+.15, col=cols[3], lwd=3, lty=3 )
...
for( ii in modes ){
	...
	box()
	mtext( side=1, expression( v[2] ), cex=1.35, line=3.2 )
	...
	for( jj in 1:7 )
	axis(1,cex.axis=1.1,at=xs[jj],lab=xs[jj])
	if( ii == modes[1] )
