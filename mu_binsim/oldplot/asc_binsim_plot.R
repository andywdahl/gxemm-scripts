...
h2s			<- array( NA, dim=c( 2, nx, maxit, 7), dimnames=list( modes, 1:nx, 1:maxit, c('g','hom','het','d1','d2','d1b','d2b') ) )
for( it in 1:maxit )
	for( mode in modes )
...
try({
	...
	savefile5	<- paste0( 'Rdata/diag2/',fs[xval], '_', it, '_', mode, '.Rdata' )
...
print( apply( ! is.na(h2s), c(1,2,4), sum )[,,c('g','hom','d1','d1b')] )

pdf( paste0( '~/figs/gxemm/Fig4.pdf' ), width=12.5, height=5.6 )
...
for( mode in modes ){
	...
	if( mode %in% c( 'ldak' ) ){
	lines1( fs, rowMeans( h2s[mode,,,'d1']	* adjs[,,'both'], na.rm=T ), col=6      , lty=2, lwd=3, pch=1 )
	lines1( fs, rowMeans( h2s[mode,,,'d2']	* adjs[,,'both'], na.rm=T ), col=3      , lty=2, lwd=3, pch=1 )
	lines1( fs, rowMeans( h2s[mode,,,'d1']	* adjs[,,'1'   ], na.rm=T ), col=cols[3], lty=1, lwd=3, pch=16)
	lines1( fs, rowMeans( h2s[mode,,,'d2']	* adjs[,,'2'   ], na.rm=T ), col=cols[4], lty=1, lwd=3, pch=16)
	lines1( fs, rowMeans( h2s[mode,,,'g']		* adjs[,,'both'], na.rm=T ), col=cols[1], lty=1, lwd=3, pch=16)
	lines1( fs, rowMeans( h2s[mode,,,'het']	* adjs[,,'both'], na.rm=T ), col=cols[2], lty=1, lwd=3, pch=16)
	#lines1( fs, rowMeans( h2s[mode,,,'het']                 , na.rm=T ), col=cols[2], lty=2, lwd=3, pch=1 )

	lines1( fs, rowMeans( h2s[mode,,,'d1b']	* adjs[,,'1'   ], na.rm=T ), col=cols[3], lty=3, lwd=3, pch=5, cex=3 )
	lines1( fs, rowMeans( h2s[mode,,,'d2b']	* adjs[,,'2'   ], na.rm=T ), col=cols[4], lty=3, lwd=3, pch=5, cex=3 )

	} else {
	lines1( fs, rowMeans( h2s[mode,,,'d1']	                , na.rm=T ), col=cols[3], lty=1, lwd=3, pch=16)
	lines1( fs, rowMeans( h2s[mode,,,'d2']	                , na.rm=T ), col=cols[4], lty=1, lwd=3, pch=16)
	lines1( fs, rowMeans( h2s[mode,,,'d1b']	                , na.rm=T ), col=cols[3], lty=3, lwd=3, pch=5, cex=3 )
	lines1( fs, rowMeans( h2s[mode,,,'d2b']	                , na.rm=T ), col=cols[4], lty=3, lwd=3, pch=5, cex=3 )
	#lines1( fs, rowMeans( h2s[mode,,,'d1c']	              	, na.rm=T ), col=5      , lty=1, lwd=3, pch=16)
	#lines1( fs, rowMeans( h2s[mode,,,'d2c']	              	, na.rm=T ), col=6      , lty=1, lwd=3, pch=16)
	lines1( fs, rowMeans( h2s[mode,,,'g']										, na.rm=T ), col=cols[1], lty=1, lwd=3, pch=16 )
	lines1( fs, rowMeans( h2s[mode,,,'het']									, na.rm=T ), col=cols[2], lty=1, lwd=3, pch=16 )
	}
}
...
for( mode in modes ){
	...
	for( ii in c( 'iid', 'h2eq', 'h2eq*', 'diag3', 'diag1' ) )
		lines1( fs, apply(allps[mode,,,ii]< .05,1,mean,na.rm=T), lty=ltys[ii], col=cols1[ii] )#, y.max=10
}
...
for( ii in modes ){
	...
	box()
	mtext( side=1, 'Population Prevalence', cex=1.35, line=3.2 )

	p1	<- rowMeans( allprev[,,'1']		, na.rm=T )
	p2	<- rowMeans( allprev[,,'2']		, na.rm=T )
	target	<- (1646/(1646+982)) / (3139/(3139+3832))
	fstar		<- which.min(abs( target - p1/p2 ))
	#abline( v=fs[fstar], col=2 )

	for( jj in 1:7 )
	axis(1,cex.axis=1.1,at=fs[jj],lab=fs[jj],col.axis=1+(jj==4))
	#if( ii == modes[1] )
